...
// single continuous DFS pass seeded from a scramble that can reach a nominal
// depth almost instantly just by descending one path without ever completing a
// full level. Uses the same orientation-lookup pruning bound Solver::dfs() uses.
...
int main(int argc, char** argv) {
	...
	IddfsResult iddfs_non_domino = iddfs_benchmark(non_domino_moves, 30, std::chrono::seconds(10));
	log_iddfs_result(std::cout, "Non domino reduced search (bmark-5)", iddfs_non_domino);

	IddfsResult iddfs_domino = iddfs_benchmark(domino_moves, 30, std::chrono::seconds(10));
	log_iddfs_result(std::cout, "Domino reduced search (bmark-5)", iddfs_domino);
